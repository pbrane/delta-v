...
Expect 36 distinct `10.0.0.x` `host` values mapped to distinct `exporter_node_id`s
(the 5-stage Clos fabric: 4 core + 8 agg + 8 edge + 16 hosts).
...
- The `nl6-lab` requisition and the device seed are both generated by
  `components/nl6/gen-fabric.py`, so every device IP (core `10.0.0.1`–`.4`, agg
  `.21`–`.28`, edge `.41`–`.48`, host `.61`–`.76`) has a matching node by
  construction — no unmapped (`exporter_node_id=0`) devices. Regenerate both
  together (`gen-fabric.py write ...`) if you change `CLOS_K`.
