...
On resource-constrained lab VMs, layer `compose.override.dev.yml` on top of the base file to shrink JVM heap / metaspace / thread-stack allocations per daemon. Saves ~2–3 GB of stack-wide RSS at lab scale (28 nodes); numbers calibrated from empirical `jcmd GC.heap_info` on the v1.2.0-alpha2 smoke test.
...
```bash
docker compose -f compose.yml -f compose.override.dev.yml --profile <profile> up -d
```

Only use the override for dev/test/demo — the production-shaped defaults in `compose.yml` are sized for real target counts. See the comment block at the top of `compose.override.dev.yml` for per-daemon sizing rationale.
