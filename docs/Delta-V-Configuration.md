...
## General thoughts:
- I have long thought that the eventconf in OpenNMS was overloaded and was carrying the load for 3 tangential requirements and should be split out into the configuration of each of their corresponding functional services
    ...
    - Alarm behavior (Eventd got highjacked [by me long] long ago to handle the Alarm behavior, before Alarmd was created, and all the Alarm workflow was handled on the Eventd thread. After Alarmd was created, the alarm-data element, still in the eventconf, should have been pulled into a separate configuration
