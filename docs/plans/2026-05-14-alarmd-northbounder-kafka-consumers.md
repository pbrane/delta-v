...
- **~~Alarm record schema evolution.~~** **Decided (PR #269 review):** `deltav-alarms-state-change` is a **public contract from day one** — versioning rules and deprecation policy apply. Third-party consumers (custom user-built forwarders, K8s operator integrations, exotic destinations) will appear quickly once the topic exists; we don't get to break the wire format casually after that.

- **~~Filtering at the broker.~~** **Decided (PR #269 review):** **single topic `deltav-alarms-state-change` with consumer-side filtering**, not per-destination topics. Matches the prometheus-writer / time-series pipeline pattern. Per-destination topics (`deltav-alarms-to-syslog`, etc.) would require alarmd to know about every downstream destination at produce time — exactly the coupling this plan retires. The small CPU cost of filtering on every consumer is the right trade for the decoupling.
